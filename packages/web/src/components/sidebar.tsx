...
export function SidebarSection({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
    return <div {...props} className={clsx(className, 'flex flex-col gap-0.5')} data-slot="section" />
}
...
export function SidebarSpacer({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
    return <div {...props} className={clsx(className, 'mt-8 flex-1')} aria-hidden="true" />
}
...
export function SidebarItem({
    current,
    className,
    children, ...props }: { current?: boolean; className?: string; children: React.ReactNode } &
    ...
    return <span className={clsx(className, '')}>
        {'href' in props ? (
            ...
            <div className={clsx('cursor-default', classes)} data-current={current ? 'true' : undefined} >
                {children}
            </div>
        )}
    </span>
